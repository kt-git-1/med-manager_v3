    ...
    if (showPicker) {
        ModalBottomSheet(onDismissRequest = { showPicker = false }, modifier = Modifier.testTag("caregiver-pdf-picker")) {
            Column(Modifier.fillMaxWidth().padding(20.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
                ...
                Row(Modifier.horizontalScroll(rememberScrollState()), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    ReportPeriodPreset.entries.forEach { candidate ->
                        FilterChip(
                            ...
                            label = { Text(periodLabel(candidate)) },
                ...
                if (state.generationFailed) Text(stringResource(R.string.caregiver_pdf_generation_failed), color = androidx.compose.material3.MaterialTheme.colorScheme.error)
                if (state.generating) LinearProgressIndicator(Modifier.fillMaxWidth())
                Button(
                    onClick = {
                        val chosen = period ?: return@Button
                        scope.launch {
                            val report = repository.loadReport(patientId, chosen.from, chosen.to) ?: return@launch
                            val generated = withContext(Dispatchers.IO) { CaregiverPdfReportGenerator.generate(context, report) }
                    ...
                    enabled = period != null && validation == null && !state.generating,
