...
- [x] Current published source is audited for every emulator-verifiable row. C122-C123 replace pre-published-build visual evidence with exact `main@432b34c` runtime pairs for UI-101 and UI-001; older images remain historical/supporting evidence until each changed surface is explicitly recaptured. Physical/TalkBack/OEM variants remain an explicit V1 gate.
