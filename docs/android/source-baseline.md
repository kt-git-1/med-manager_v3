...
| Today zero-dose state | A zero-length Today response does not by itself mean that no medication is registered. When an active, unarchived scheduled medication exists but has no dose today, Today shows a neutral no-schedule state and never prompts medication registration. | Patient Today already uses a neutral empty state. Caregiver Today retains `hasScheduledMedications` from the medication response and separates no-schedule from true no-medication onboarding. Both short zero-dose surfaces remain pull-to-refresh capable. |
...
- [x] Current published source is audited for every emulator-verifiable row. C122-C124 replace pre-published-build visual evidence with exact `main@432b34c` runtime pairs for UI-101, UI-001 and UI-104; older images remain historical/supporting evidence until each changed surface is explicitly recaptured. Physical/TalkBack/OEM variants remain an explicit V1 gate.
