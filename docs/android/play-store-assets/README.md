...
Exact source mapping is machine-readable in [`phone-ja-JP/sources.tsv`](phone-ja-JP/sources.tsv), and Japanese alt text is maintained in [`../play-store-listing-ja.md`](../play-store-listing-ja.md). C122-C124 replace screenshots 02, 01 and 03 with exact Patient Today, Mode Select and Patient History fixtures recaptured against the unmodified published iOS 1.0.6 Build 51 runtime. The other five C107 sources remain deterministic post-parity Compose evidence, but they predate this stricter published-build recapture procedure and stay queued for the same audit. If UI behavior changes, recapture the deterministic source fixture, update the mapping and regenerate the corresponding store image; do not edit medical state or copy only in the marketing export.
