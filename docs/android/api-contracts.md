...
- C98 provides a database-enforced read-only observation tool for that production boundary. `preflight` succeeds only when the Prisma record, both columns and both indexes are absent; `postdeploy` succeeds only for one finished/non-rolled-back record whose checksum matches the checked-in migration, exact valid/ready patient-scoped indexes and zero duplicate non-null patient/key groups. It reports only total/keyed row counts and never identifier values.
- C97/C98 are upgrade and observation gates, not deployment evidence. The production operator must still choose an appropriate window, run the normal migration command once, execute C98 before and after under explicit remote/TLS confirmation, and verify the deployed API plus real uncertain-response recovery on approved test identities.
