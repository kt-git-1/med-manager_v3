...
- Supabase error parsing follows current iOS message-first then HTTP-status classification: invalid credentials, confirmation delivery/required, duplicate email, weak password, invalid email, credential check, forbidden, endpoint unavailable, rate limit and service unavailable all resolve to pinned Japanese resources. Network failures use the current iOS retry copy rather than exposing transport details.
...
- The confirmation view enables resend with a 60-second countdown. Signup preserves the full typed iOS auth failure contract; resend uses its own iOS-specific 429 copy, preserves typed/network failures and maps only unknown exceptions to the generic resend message.
...
- Compose tests cover choice content, independent login/signup actions, production navigation to both forms, canonical form fields, typed signup-validation/confirmation rendering, 200% reachability, loading duplicate-submit locks and transient-state reset after leaving and re-entering an auth form. Stable field/submit/navigation semantics tags avoid placeholder-child ambiguity in UI automation.
