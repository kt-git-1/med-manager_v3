...
- [ ] V01 Complete the remaining assisted spoken TalkBack traversal, real destructive/mutation-interruption paths, API 26-28 target, Google/reference target, Firebase/FCM and exact signed-Play artifact rows. C65-C75 cover authenticated appearance, core lifecycle, permission, local reminder lifecycle, legal-link/link-code I/O, the safe A302SH IME matrix, read-only offline recovery, same-installation role isolation, synthetic single-flight/cancellation hardening and local PRN/inventory server-idempotency contracts on one current non-Google OEM/API 35 Debug device; they do not close migration deployment, real interrupted-write recovery, signed Play, FCM or cross-device acceptance.
