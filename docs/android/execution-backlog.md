...
- [ ] V01 Complete the remaining assisted spoken TalkBack traversal, authenticated create/edit-form IME, destructive/mutation-interruption paths, API 26-28 target, Google/reference target, Firebase/FCM and exact signed-Play artifact rows. C65-C72 cover authenticated appearance, core lifecycle, permission, local reminder lifecycle, legal-link/link-code I/O, Patient-link OEM IME, safe read-only offline recovery and same-installation role isolation on one current non-Google OEM/API 35 Debug device; they do not close signed Play, FCM or cross-device acceptance.
