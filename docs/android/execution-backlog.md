...
- [ ] V01 Complete the remaining assisted spoken TalkBack traversal, real destructive/mutation-interruption paths, API 26-28 target, Google/reference target, processed Analytics Events/Explore, Firebase FCM and exact signed-Play artifact rows. C65-C78 cover authenticated appearance, core lifecycle, permission, local reminder lifecycle, legal-link/link-code I/O, the safe A302SH IME matrix, read-only offline recovery, same-installation role isolation, synthetic single-flight/cancellation hardening, local PRN/inventory server idempotency, physical Analytics consent/DebugView/Realtime, both Android FCM event envelopes and one-device FCM token acquisition/cleanup; they do not close migration deployment, real interrupted-write recovery, signed Play, authenticated FCM registration/delivery or cross-device acceptance.
...
- [ ] Recheck the processed Events report and create/remove the temporary fixed-enum-only Explore after aggregation delay before promoting `XP-004` to `VERIFIED`.
...
Execute the **Gate I physical-device and Play release matrix** using `play-release-runbook.md` once its external inputs exist. C63 closes the complete cross-API emulator matrix at 272/272 on API 26/33/35 (816/816 total); C64 adds the current requirement audit, JVM/Lint/Debug/Release/APK/asset gates, a fresh API-35 full run and the affected medication-form 25/25 slice on all three APIs. Production Firebase values, a release-owner-managed upload key, Play Console access and physical devices remain required; do not mark device-only rows verified from emulator evidence.
