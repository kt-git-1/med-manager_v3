...
- [ ] V01 Complete the remaining assisted spoken TalkBack traversal, full IME/browser/share/destructive/network paths, API 26-28 target, Google/reference target, Firebase/FCM and exact signed-Play artifact rows. C65-C68 cover authenticated appearance, core lifecycle, permission and exact local primary/secondary/task-removed/Doze/cancellation behavior on one current non-Google OEM/API 35 Debug device; they do not close signed Play, FCM or cross-device acceptance.
