...
- [ ] V01 Complete the remaining assisted spoken TalkBack traversal, real destructive/mutation-interruption paths, API 26-28 target, Google/reference target, processed Analytics Events/Explore, Firebase FCM and exact signed-Play artifact rows. C65-C79 cover authenticated appearance, core lifecycle, permission, local reminder lifecycle, safe I/O/IME/offline/role isolation, mutation hardening, physical Analytics, both Android FCM envelopes, token acquisition/cleanup and the authenticated production registration failure boundary. They do not close migration deployment, real interrupted-write recovery, signed Play, FCM registration/delivery until the Android API contract is deployed, or cross-device acceptance.
