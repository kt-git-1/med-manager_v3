...
- [x] C87 Enable strict dependency locking only for `productionReleaseRuntimeClasspath`; check in the complete generated lock state; make it an explicit Release SDK policy input; prove the checked-in state resolves and missing state fails closed; document the intentional dependency-update/review procedure without locking unrelated Debug/test configurations.
...
- [x] C125 Separate a true no-medication Today state from an active scheduled medication with zero doses today. Caregiver Today now shows neutral next-schedule guidance without a registration CTA in the latter case; Patient and Caregiver zero-dose surfaces both have emulator-verified pull-to-refresh regressions.
...
- [ ] V01 Complete the assisted spoken TalkBack traversal, real destructive/mutation-interruption paths, old-supported physical target, Google/reference target, Firebase FCM and exact signed-Play artifact rows. C65-C125 plus the 2026-08-23 H07 acceptance cover authenticated appearance, core lifecycle, permission, local reminder lifecycle, safe I/O/IME/offline/role isolation, mutation hardening, Analytics through Android-only Explore cleanup, isolated legacy-upgrade/read-only deployment-audit and manual fail-closed production-release/control-plane contracts, Play policy/review-access, reproducible listing assets, signed-artifact contracts and current SDK/manifest policies. They do not close spoken real-finger operation, production GitHub environment protection, server-side SSL enforcement, verified Play organization ownership/legal-name publication, authorized production migration/deployment, final review credentials, real interrupted-write recovery, release-owner-signed/uploaded/published/generated/downloaded Play bytes or installed receipt execution, live URL/Play preview, current vendor-disclosure/Console review, App Links/FCM production verification, dependency findings, closed-test crash/ANR monitoring, cross-device physical acceptance or the remaining published-build UI recaptures.

### C99 canonical residual release gates

- [x] RG-001 Firebase Analytics Explore verification
...
- [x] Create/remove the temporary fixed-enum-only Explore under explicit Console-write control; the 2026-08-23 evidence verifies Android-only safe rows and records cleanup, so `XP-004` is `VERIFIED`.
