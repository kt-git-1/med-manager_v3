...
Execute the **Gate I physical-device and Play release matrix** using `play-release-runbook.md` once its external inputs exist. C63 closes the current emulator-verifiable published-source audit with 202/202 JVM tests, Lint and 272/272 UI tests on each of API 26/33/35 (816/816 total). Production Firebase values, a release-owner-managed upload key, Play Console access and physical devices remain required; do not mark device-only rows verified from emulator evidence.
