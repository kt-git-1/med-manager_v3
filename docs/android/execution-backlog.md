...
- [ ] V01 Complete the remaining assisted spoken TalkBack traversal, real destructive/mutation-interruption paths, API 26-28 target, Google/reference target, Firebase/FCM and exact signed-Play artifact rows. C65-C74 cover authenticated appearance, core lifecycle, permission, local reminder lifecycle, legal-link/link-code I/O, the safe A302SH IME matrix, read-only offline recovery, same-installation role isolation and synthetic single-flight/cancellation hardening on one current non-Google OEM/API 35 Debug device; they do not close signed Play, server-idempotency, FCM or cross-device acceptance.
