...
- [ ] V01 Complete the remaining manual TalkBack traversal, authenticated Patient/Caregiver physical dark/system-200%-font flows, exact task-card removal, API 26-28 target and Google/reference target. C65-C66 cover only unauthenticated/local-state behavior on one current non-Google OEM/API 35 device and do not close Play/FCM/notification-delivery rows.
