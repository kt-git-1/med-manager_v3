...
- [ ] V01 Complete the remaining assisted spoken TalkBack traversal, real destructive/mutation-interruption paths, API 26-28 target, Google/reference target, processed Analytics Events/Explore, Firebase FCM and exact signed-Play artifact rows. C65-C77 cover authenticated appearance, core lifecycle, permission, local reminder lifecycle, legal-link/link-code I/O, the safe A302SH IME matrix, read-only offline recovery, same-installation role isolation, synthetic single-flight/cancellation hardening, local PRN/inventory server idempotency, physical Analytics consent/DebugView/Realtime and both Android FCM event envelopes; they do not close migration deployment, real interrupted-write recovery, signed Play, physical FCM or cross-device acceptance.
