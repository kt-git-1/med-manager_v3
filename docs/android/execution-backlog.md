...
- [ ] V01 Complete the remaining assisted spoken TalkBack traversal, full remaining IME/destructive/network paths, API 26-28 target, Google/reference target, Firebase/FCM and exact signed-Play artifact rows. C65-C69 cover authenticated appearance, core lifecycle, permission, local reminder lifecycle and legal-link/link-code I/O on one current non-Google OEM/API 35 Debug device; they do not close signed Play, FCM or cross-device acceptance.
