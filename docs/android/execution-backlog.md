...
- [ ] V01 Complete the remaining assisted spoken TalkBack traversal, full IME/browser/share/destructive/network paths, exact timed/secondary/Doze notification delivery, API 26-28 target and Google/reference target. C65-C67 now cover authenticated appearance, core lifecycle, permission and diagnostic local-notification behavior on one current non-Google OEM/API 35 Debug device; they do not close signed Play, FCM or remaining exact-delivery rows.
