...
6. exactly the reviewed 37-file API allowlist covering Android FCM registration/envelopes, mutation idempotency, privacy/security, Digital Asset Links, production release/configuration and their tests/migration;
...
Current policy note: C97 adds `.github/workflows/api-ci.yml` and two reviewed mutation-migration verifier scripts; C98 adds two reviewed read-only deployment-audit scripts; C100 adds the reviewed manual production workflow, two production-release contract scripts and `api/vercel.json`. The current boundary is exactly three workflows and 37 API files. Every future expansion remains a direct review event.
