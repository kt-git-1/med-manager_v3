...
C06 remains open. Primary Patient and Caregiver screens now have explicit 1.3/2.0 action reachability coverage, including Caregiver Settings/tutorial, and the production Patient preview passes Activity recreation plus portrait/landscape transitions. Full TalkBack traversal on every state, dark/large-text state captures, matched iOS variants, Caregiver configuration transitions, additional compact/large device-class evidence and physical-device verification remain. Those residuals are intentionally not inferred from emulator automation.
