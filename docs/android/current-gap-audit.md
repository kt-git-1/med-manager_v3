...
| P2 | Production artifact ownership/configuration is pending | Firebase runtime values and fail-closed validation now exist; C84 rejects Supabase secret/service-role client credentials; C85 verifies signed-AAB signature/upload-certificate match; C86-C88 lock and gate SDK/APK manifest policy; C89 validates the generated AAB structure and protobuf manifest; C90 additionally proves the selected upload alias/private key/certificate before generation and again checks the artifact signer | Release owner supplies approved runtime/upload key/fingerprint, then builds the exact verified signed AAB, rechecks current SDK disclosures and completes Play scan |
