...
| P2 | Production artifact ownership/configuration is pending | Firebase runtime values and fail-closed validation now exist; C84 rejects Supabase secret/service-role client credentials; the Release APK passes application-ID, SDK, forbidden-permission and 16 KB ZIP/ELF checks | Release owner supplies approved runtime/signing inputs, then verifies the exact signed AAB and Play scan |
