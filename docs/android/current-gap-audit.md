...
| Resolved C96 | Long-lived Android branch could overwrite newer iOS/API work at merge | Fresh `origin/main` is the exact ancestor; committed delta is confined to Android, `docs/android`, reviewed Android CI/ignore and an exact 30-file API allowlist with zero `ios/` paths. One accepted/nine rejected Git fixtures and hosted ancestry fetch protect the boundary | Rerun after every main advance; never weaken the allowlist or merge while the task fails |
