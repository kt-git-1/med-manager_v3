...
| Resolved C96 | Long-lived Android branch could overwrite newer iOS/API work at merge | Fresh `origin/main` is the exact ancestor; committed delta is confined to Android, `docs/android`, reviewed CI/ignore and exact API allowlists with zero `ios/` paths. C97 deliberately expands the original one-workflow/30-file contract to two workflows/32 files. One accepted/ten rejected Git fixtures and hosted ancestry fetch protect the boundary | Rerun after every main advance; treat every allowlist change as review work and never merge while the task fails |
