...
The Android project contains production Patient and Caregiver flows through Gate G plus the automated portion of privacy-first Analytics in Gate H. C61 safely merged published `main@432b34c` into `android-dev` and revalidated the API contract. C62-C64 corrected stale tutorial/UI assumptions and audited all 69 parity rows, leaving six external-only release rows. C65-C73 execute the physical gate on one SHARP A302SH non-Google OEM target at Android 15/API 35. C74 hardens every health/destructive mutation owner with one repository-level nonblocking single-flight boundary and explicit cancellation cleanup without automatic replay. C75 closes the discovered local server gap for scheduled individual-dose concurrency plus Android PRN/inventory response-loss retry by adding sole-insert ownership and optional UUID-v4 client mutation keys while preserving legacy iOS requests. The latest gates pass API 332/332, A302SH UI 278/278, Debug JVM 216/216, Release JVM 213/213, Lint, Debug/Release assembly, Release APK compatibility and Play assets. No production mutation or schema deployment was executed. Live Firebase, complete spoken TalkBack, two remaining physical device classes, production migration plus real destructive/mutation-interruption evidence and Play release operations remain separate gates.
...
| Physical release verification | PARTIAL | One current non-Google OEM Debug target has authenticated role, appearance, lifecycle, permission, local alarm/Doze/standby/tap/cancellation, legal-link/link-code I/O, read-only cached/cold offline recovery, same-installation role isolation and a post-C75 synthetic UI regression. Client single-flight and PRN/inventory API idempotency are locally hardened; production migration and real destructive/mutation interruption are not physically proven. Signed Play, old/reference devices, spoken TalkBack and Firebase remain |
