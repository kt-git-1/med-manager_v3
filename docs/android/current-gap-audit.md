...
| P2 | Production artifact ownership/configuration is pending | Firebase runtime values and fail-closed validation now exist; C84 rejects Supabase secret/service-role client credentials; C85 verifies signed-AAB structure/signature/upload-certificate match and signed/unsigned APK compatibility with synthetic inputs; C86 gates the exact Release SDK inventory and APK advertising/attribution permission exclusions; C87 strictly locks that reviewed graph; C88 verifies the actual merged APK manifest security/privacy surface | Release owner supplies approved runtime/upload key/fingerprint, then builds the exact verified AAB, rechecks current SDK disclosures and completes Play scan |
