...
| P2 | Production artifact ownership/configuration is pending | Firebase runtime values and fail-closed validation now exist; C84 rejects privileged client keys; C85 verifies signed-AAB signature/upload-certificate match; C86-C89 lock and gate SDK/APK/AAB policy; C90 proves the selected key before generation and artifact signer after; C91 atomically records the clean source/version/AAB/certificate/dependency identity | Release owner supplies approved runtime/upload key/fingerprint, builds the exact verified signed AAB plus JSON ledger, independently compares Play, rechecks current SDK disclosures and completes Play scan |
