...
After installing the exact Internal-test artifact from Google Play on an API-31+ physical device, run the C119 installed-package gate. It first reruns the production gate, then requires Google Play installer identity, exact C118 version/signing/base-APK bytes, at least one optimized split and only `www.okusuri-mimamori.com: verified`; it never launches or mutates app data. Keep both receipt paths in the release-owner evidence area outside Git:
...
`verifyAndroidCiRuntimeContract` protects the hosted runner used for those claims. Android CI permits only the full-SHA-pinned official checkout v6.1.0, setup-java v5.7.0 and setup-gradle v6.3.0 Node 24 release commits in that order; it grants only `contents: read`, persists no checkout credential, fetches full ancestry and keeps Temurin 17. The contract rejects mutable tags, SHA/comment/order drift, an extra action, write permission, privileged trigger, runner/branch/configuration drift and noncanonical workflow bytes. Before final merge, inspect the exact hosted run annotations as well as its conclusion: a green job with an action-runtime deprecation warning does not satisfy C115/RG-010.

C121 applies the same acceptance standard to all API workflows after the exact C120 run exposed four Node 20 deprecation warnings despite green conclusions. API CI, API E2E and Android API Production may use only full-SHA checkout v7.0.1, setup-node v7.0.0 and upload-artifact v7.0.1, whose official `action.yml` files declare `node24`. They grant only `contents: read`, retain no checkout credential and expose exactly thirteen reviewed action occurrences. Inspect every exact API CI/E2E job annotation after each action-pin change; zero warnings is required. Never replace the full SHA with a mutable major tag or ignore a skipped failure-only upload action when auditing the source.

The final signed task also requires clean committed Android and consumed role/icon/store inputs. Its schema-v2 ledger includes the Japanese listing, source map, icon, feature graphic and ordered eight screenshot SHA-256 values; handoff creation re-hashes those exact repository files. C111-C112 run this production generator-to-handoff path in hosted CI using an isolated committed tree and disposable `jarsigner`/`keytool` identity, including dirty-listing and between-phase store-change rejection; that signer is never a production credential. The owner task creates `app/build/outputs/play-release/v<version>-code<versionCode>-<commit12>/` atomically. That directory must contain exactly the commit/version-named AAB, byte-identical `play-release-evidence.json` and `SHA256SUMS`. If the same target already exists it must match all three files; the task never silently overwrites a conflict.

Immediately before upload, from `android/`, reverify the retained directory against the current repository instead of relying on a generic checksum command alone:

```bash
python3 scripts/verify-prepared-play-release-handoff.py \
...
After C116 passes, call the read-only `generatedapks.list` endpoint for the exact package and C114/C116 `versionCode`: `GET /androidpublisher/v3/applications/com.afterlifearchive.medmanager/generatedApks/<versionCode>`. Retain only the raw JSON response under owner control. Then bind its Play-generated signing identities and base split metadata to the same chain:

```bash
python3 scripts/verify-play-generated-apks-receipt.py \
  --handoff app/build/outputs/play-release/v<version>-code<versionCode>-<commit12> \
  --bundle-response /private/path/play-bundle-response.json \
  --upload-receipt app/build/outputs/play-upload-receipts/v<version>-code<versionCode>-<commit12>.play-upload-receipt.json \
  --release-list-response /private/path/play-internal-qa-releases-response.json \
  --track-response /private/path/play-internal-qa-track-response.json \
  --internal-track-receipt app/build/outputs/play-track-receipts/v<version>-code<versionCode>-<commit12>.play-internal-track-receipt.json \
  --generated-apks-response /private/path/play-generated-apks-response.json \
  --expected-app-signing-sha256 "$PLAY_APP_SIGNING_CERT_SHA256_FINGERPRINTS" \
  --output app/build/outputs/play-generated-apk-receipts/v<version>-code<versionCode>-<commit12>.play-generated-apks-receipt.json \
  --repository-root ..
```

C117 reruns the complete C116 chain, requires production targeting and at least one base-master split under every generated signing-key group, then compares the complete normalized API certificate set with the independently read App Links set. Any reuse of the retained upload certificate fails. The receipt excludes download IDs/request URLs and retains only public fingerprints, hashes and counts. It proves metadata identity only.

For every C117 signing-key group, select exactly one base-module master row (`moduleName=base`, empty `splitId`) and call `generatedapks.download` with its `downloadId`. Save the response body as an APK in the owner-controlled evidence area, never in Git or the immutable three-file handoff. Repeat `--downloaded-base-apk DOWNLOAD_ID APK` once per signing-key group:

```bash
python3 scripts/verify-play-downloaded-base-apks-receipt.py \
  --handoff app/build/outputs/play-release/v<version>-code<versionCode>-<commit12> \
  --bundle-response /private/path/play-bundle-response.json \
  --upload-receipt app/build/outputs/play-upload-receipts/v<version>-code<versionCode>-<commit12>.play-upload-receipt.json \
  --release-list-response /private/path/play-internal-qa-releases-response.json \
  ...
  --generated-apks-response /private/path/play-generated-apks-response.json \
  --expected-app-signing-sha256 "$PLAY_APP_SIGNING_CERT_SHA256_FINGERPRINTS" \
  --generated-apks-receipt app/build/outputs/play-generated-apk-receipts/v<version>-code<versionCode>-<commit12>.play-generated-apks-receipt.json \
  --downloaded-base-apk '<base-master-download-id-for-key-1>' /private/path/base-master-key-1.apk \
  --output app/build/outputs/play-downloaded-apk-receipts/v<version>-code<versionCode>-<commit12>.play-downloaded-base-apks-receipt.json \
  --repository-root ..
```

C118 reruns C117 byte-for-byte and refuses missing rotation coverage, unknown/configuration/duplicate download IDs or reused files. Every APK must be a bounded, unique, traversal-free ZIP outside the handoff with manifest and DEX and without private configuration/key material. SDK `aapt2` must report the production package, exact C117 `versionCode`/`versionName` and no nonempty split identity; SDK `apksigner` must verify exactly one signer, its assigned C117 certificate and an embedded v2-or-newer scheme. The receipt excludes download IDs and paths, retaining only fixed identity, hashes, counts, public fingerprints and Build Tools version. It still does not prove the Google Play installer, fresh install/update or device behavior.
...
- The exact-version `generatedapks.list` response contains production base-master split metadata under the complete independent App Links app-signing certificate set; the C117 receipt is retained outside the handoff and the upload certificate is not in that set.
- One real `generatedapks.download` base-master APK per C117 signing key passes C118 package/version/ZIP/DEX/signer/embedded-signature checks; its path-free receipt is retained outside the handoff with the owner-controlled APK bytes.
- The exact Play-installed physical package passes C119 against that unchanged C118 receipt: its installed `base.apk` hash/size matches exactly one row, version/signing set/Google Play installer/split inventory remain stable and the canonical App Link is verified. Retain the serial/path-free C119 receipt outside Git.
...
1. Upload the signed AAB to Internal testing through the approved owner-controlled path, pass the real Bundle response through C114, commit the track change, then pass the post-commit `qa` releases-list and fresh inspection-edit Track responses through C116. Delete the unmodified inspection edit. Pass exact-version generated APK metadata through C117, download one base-master per signing-key group and pass their bytes through C118. Retain all raw responses/APKs/receipts under owner control. Record commit SHA, `versionCode`, upload/APK SHA-256, certificate fingerprints, tester account and result outside Git.
2. Install from Play, not adb. Run `verifyPlayInstalledPackageReceipt` with the unchanged C118 receipt and retain C119, then verify caregiver/patient sign-in, session restoration, both production auth paths, browser fallback, FCM permission/token/delivery/tap, local reminders, background/Doze/process death, legal links and analytics consent.
...
- No real exact-version `generatedapks.list` response exists yet; C117's synthetic fixtures prove only metadata parsing, signing-set/upload-key separation and secret-free receipt behavior.
- No real `generatedapks.download` base-master APK set exists yet; C118's synthetic fixtures and local Debug-APK SDK integration prove only the fail-closed byte/tool contract, not Play signing, installer identity or installation.
