...
The final signed task also requires clean committed Android and consumed role/icon/store inputs. It atomically writes `app/build/reports/play-release-evidence.json` with the full commit, branch, pinned iOS/API baseline, package/version/SDK identity, exact AAB and dumped-manifest hashes, upload certificate, module/DEX/native counts, dependency-lock and SDK-inventory hashes/counts and the passed gate names. Copy that JSON alongside the exact AAB without editing either. If the working input tree changes, rebuild both; never pair a prior report with a later artifact.
...
- The JSON ledger SHA/certificate/version/commit exactly match the AAB selected for upload; retain both together in the release evidence package.
