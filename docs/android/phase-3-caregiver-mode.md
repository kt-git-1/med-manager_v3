...
- Directly rechecked the shipping iOS files at `staging@2b7d1fe`; their hashes match the pinned Android-worktree copies. `AppConstants.billingEnabled=false` and Android `BILLING_ENABLED=false` both remove the caregiver export entry in the initial release, while Patient mode never owns an export entry.
