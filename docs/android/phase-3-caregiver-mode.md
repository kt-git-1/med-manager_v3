...
- The production Today tab now renders the iOS information hierarchy: patient header, missed warning, the fixed four-column `今日の服薬状況` overview, PRN entry and morning/noon/evening/bedtime timeline. The overview shows the actual recorded time for taken/late slots and the configured time otherwise, with distinct late/taken/partial/missed/pending/no-plan states. Empty, no-patient, no-selection, loading and retry states use the shared caregiver copy.
- A partially recorded overview column uses the same orange partial state as the detailed slot and states `recorded/total種類`, so the count cannot be mistaken for a dose or event count.
- Progress is medication-based (`recorded/total`), not completed-slot based. A mixed slot stays visibly `一部記録`; its taken medication remains recorded while only pending/missed medications remain unresolved, and an inventory-insufficient medication retains its own warning.
...
F06 and `CG-009` are `IMPLEMENTED`. The UI-204/UI-205 rebaseline covers the current-iOS loading/updating lifecycle, action-first guide, empty onboarding, sectioned list, status help, one-week quick refill, cadence/status/large-quantity detail header, refill plan, settings and adjustment hierarchy. Successful detail actions dismiss to the authoritative list; failure preserves the form with exact-action retry. Direct API-35 light captures cover both surfaces. F07 owns the explicit cross-tab revision matrix before Gate F closes.
...
- The month surface mirrors the current iOS hierarchy with patient identity, previous/current-month navigation, Sunday-first calendar, four scheduled-slot status dots, PRN marker and semantic legend. The former Android-only day bottom sheet has been removed: selected-day counts/help and one timestamp/name-sorted scheduled/PRN timeline continue inline below the calendar, with recorder attribution, exact empty/loading/retry states and Tokyo date/time presentation. Selecting the same date again is idempotent and cannot discard visible detail/error state.
- Additive month-history `slotProgress` counts allow mixed slots to render `一部記録` and exact medication counts without changing the legacy `slotSummary` enum or stored DoseRecord status.
...
- Exact paths/auth/bodies, response aliases, patient isolation, retention separation, backfill success/failure revisions and strict push parsing have JVM/API coverage. Production Compose tests cover the message-bearing month/day loaders, timestamp timeline, exact empty/failure retry states, blocking backfill overlay, inline calendar-to-day/backfill, exact remote slot highlight and cross-patient History-tab navigation; the C16 API-35 matrix records UI-206 day parity.
...
- Android generates an A4 multipage PDF entirely on-device with Japanese text and replaces older cache output. The later C55 current-runtime reconciliation supersedes the original G02 layout with the shipping iOS summary-first/two-page, count/adherence and exact detail-label contract.
...
- Firebase Messaging is included without a checked-in `google-services.json`. Runtime values use `FIREBASE_APP_ID`, `FIREBASE_API_KEY`, `FIREBASE_PROJECT_ID` and `FIREBASE_SENDER_ID`; a build with missing values remains runnable and reports the unavailable notification configuration in Settings. Push-device registration uses the explicit `PUSH_DEVICE_ENVIRONMENT` runtime value (`DEV` or `PROD`, default `PROD`) instead of inferring it from the Debug/Release build type, because a Debug client may legitimately target the deployed Staging API whose device partition is `PROD`.
...
- The caregiver Settings switch persists device intent, registers the current token through caregiver-authenticated `POST /api/push/register` with `platform=android` and the API-matched `PUSH_DEVICE_ENVIRONMENT`, re-registers refreshed tokens and shows syncing/configuration/failure states.
...
- C79 runs the authenticated Settings path and proves the client reaches backend sync with a real token, but production `main@432b34c` still validates only `platform=ios` and returns 422 before upsert. The current branch's `android` validator and tests must be merged/deployed before FC-001 can pass. Falling back to `ios` is forbidden because platform owns the Android data-only privacy branch.
...
- Issued linking codes now move from the former Android-only inline expansion into a current-iOS-style dedicated sheet with six digit cells, copy/share, Tokyo expiry and copy confirmation. Dismissal clears only the ephemeral code.
...
- Android opens patient creation from the empty-state CTA in a modal sheet, disables blank submission and keeps failure retry in the sheet. Staging QA extended first-run onboarding so pages 7–9 perform three explicit actions inside the ten-page sequence: patient creation, real linking-code issue and first-medication registration. See `guided-caregiver-onboarding.md`.
...
- The isolated billing-enabled Android component now mirrors the current iOS menu choice and exact Japanese copy rather than clipping a horizontal chip strip. It has an explicit close action, five presets, custom dates, Tokyo-inclusive day count and future/reversed/90-day validation, with a vertically scrollable 200%-text surface.
- Network fetch through on-device rendering remains one blocking operation. On-device PDF or Sharesheet failure is caught and leaves the picker open with retry feedback; network/retention failures retain their typed repository paths. Successful output stays in private cache and shares only through a temporary FileProvider URI.
- The A4 generator now follows the iOS summary-first contract: generated timestamp, scheduled status counts, PRN count and adherence over TAKEN+MISSED only, followed by a separate detail page with iOS slot/status/recorder/time wording. The old one-page mixed summary/detail and all-scheduled adherence denominator were removed.
- C55 evidence records the free lock, light picker, dark 200% picker and a real `PdfRenderer` image under `docs/android/evidence/c55-20260716/`. Focused UI-207 coverage passes 10/10; the full API-35 suite passes 256/256, JVM passes 186/186, and Debug/Release assembly plus Lint pass.
