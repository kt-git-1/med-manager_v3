...
| UI-101 Today | current `PatientTodayBaseView` / list / view model | full initial loading and generic failure; empty, typical, long names, pending/taken/missed, insufficient, pull-to-refresh that retains cached content, success/partial, non-destructive post-write refresh failure and notification highlight. A partially recorded slot remains explicit after Toast dismissal: the strip shows `recorded/total`, and the record card shows the slot count and medication-level taken/insufficient state. |
...
| UI-104 Patient history | current `HistoryMonthView.patientSimpleHistory` | title/subtitle, today progress ring and pills; supplementary `連続記録` card with server count/qualifier/today-status action copy; Monday-first current week; recent today/yesterday summaries; loading/retry/retention. C124 retains the exact published default pair and closes filled-clock, 34/17 header, streak-accent/dark-teal and `5/7日` fixture drift while preserving production seven-day/`回分` behavior. Streak failure must not replace usable history. |
...
| UI-201 Today | `CaregiverTodayView.swift` | no patient; neutral initial loading; blocking data-unavailable card with Retry and return-to-login; exact three-step empty onboarding; patient header; optional missed alert with scheduled/actual/delay details; fixed four-column `今日の服薬状況` card whose cells show morning/noon/evening/bedtime, state icon, and actual-or-scheduled time; explicit `recorded/total種類` for partial slots; full-navigation PRN list/confirmation with orange capsule and teal proxy action; four-slot timeline with pending/taken/missed/no-plan and row-owned individual/bulk/delete actions; insufficient inventory; mutation-success/refresh-failure preservation. PRN failure stays in route and Back clears transient feedback. No next-dose hero, top medicine list/action, orange next-row state or bottom-sheet PRN chooser. C37/C49 hold the matched populated, exceptional, dark and adaptive evidence; physical TalkBack remains. |
| UI-202 Medication list | `MedicationListView.swift` | neutral initial loading; blocking recovery with Retry and return-to-login; actionable no-patient and patient-selection-required states; patient header; Add only when populated; 2 x 2 metrics; all/scheduled/PRN/ended filters; blank filtered result; large pills empty identity with exact three illustrated onboarding rows and full-width Add; long names; inventory status; updating. C24/C50 hold the matched typical, exceptional, dark and adaptive evidence; physical TalkBack remains. |
| UI-203 Medication form | `MedicationFormView.swift` | add/edit hero and completion; grouped name/strength/unit/per-dose; scheduled/PRN help; period before schedule; start date and optional end date; schedule guide; daily/weekday segment; seven weekday choices; 2 x 2 preset slots; creation-only initial inventory; notes; aggregate validation; blue save; confirmation-protected destructive delete; keyboard and unsaved dismissal. C25/C51 hold typical, state, dark and adaptive evidence; physical TalkBack/IME remains. |
| UI-204 Inventory list | `InventoryListView.swift` | neutral loading; retry/login recovery; no-patient registration; box-identified selection routing; selected-patient header; illustrated three-step empty onboarding; four metrics; priority refill guide; all/low/out filters including ended zero-stock; active/ended/unconfigured sections; quick refill/detail; blocking update. C26/C52 hold typical, lifecycle, dark and adaptive evidence; physical TalkBack/real API remains. |
| UI-205 Inventory detail | `InventoryDetailView.swift` | centered medication/status/remaining card; live tracking toggle; scheduled and PRN quantity; explicit refill preset/custom selection; refill and absolute correction confirmation; tracking-off action disabling; exact-operation failure retry below the attempted operation; blocking app-illustration update surface. C27/C53 hold typical, operational, exceptional, dark and adaptive evidence; physical TalkBack/real API remains. |
| UI-206 Caregiver history | `CaregiverHistoryView.swift`, `HistoryMonthView.swift`, `HistoryDayDetailView.swift` | full patient-list failure/no-patient/selection recovery; 62/50 patient avatar and 34/17 header; centered month retained through loading/failure with navigation only when billing-enabled; Monday-first 54-unit explained calendar; teal/red/gray/purple markers; selected-day `%d/%d回分 記録済み`; day loading/unboxed empty/full recovery; expandable scheduled time-slot groups plus one expandable purple PRN group containing all of that day's PRN records, times, quantities and recorder identities; confirmation-protected missed backfill; blocking update; exact push highlight and stale/retention/PDF foundations. C16/C28/C54 hold typical, lifecycle, operational, dark and adaptive evidence; physical TalkBack/real API remains. |
| UI-207 PDF period/share | `PDFExportButton.swift`, `PDFExportLockView.swift`, `PeriodPickerSheet.swift`, `PeriodPickerViewModel.swift`, `PDFGenerator.swift` | initial-release billing-off absence and zero Patient-mode export UI; caregiver-only billing-enabled action; unknown entitlement refresh; exact free lock copy; native five-option period menu; Tokyo-inclusive range and future/reversed/90-day validation; explicit close; blocking generation; retryable API/device/share failure; content-URI Sharesheet; private-cache cleanup. A4 output starts with an iOS-contract summary page, always starts detail on page 2, and matches status counts, PRN count, TAKEN+MISSED adherence denominator, slot/status/recorder/time labels and empty-day output. C55 holds lock, picker, dark-200% and actual rendered-PDF evidence; physical Sharesheet/TalkBack remains. |
...
- Provide top-edge pull-to-refresh on every server-backed primary content screen: Patient Today/History and Caregiver Today/Medication/Inventory list/Inventory detail/History/Settings.
- Do not add it to authentication, tutorials, editors, confirmation sheets, or Patient Settings because those screens do not present refreshable server collections.
- Keep the last rendered content visible under the native Material refresh indicator. Blocking overlays are reserved for mutations.
- Ignore a new pull while an initial load, refresh, mutation, or selected-day request is already running.
- History refreshes both its collection and the currently expanded or selected day detail.
...
Patient and Caregiver medication-recording success feedback uses the same native Android Toast contract. Do not leave persistent success text inside Today or History content; partial-record warnings may use a long-duration Toast.
...
- The app baseline is 1.25x the Android system font scale in Patient and Caregiver modes; do not opt individual screens out of the baseline.
