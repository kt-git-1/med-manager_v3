...
| XP-004 | Privacy-first Firebase Analytics parity | NOT_STARTED | Collection off by default; explicit consent; fixed enums only; no identity/health/free-text/token values; reset on disable; preview/test suppression; verify DebugView, Realtime, Events and Explore |
| XP-005 | Dynamic type/font scale and screen-reader semantics | PARTIAL | Current patient-history heading/summary content, tutorial pane semantics and 200% tutorial action reachability are covered; full patient screen audit, caregiver surfaces and physical TalkBack verification remain |
| XP-006 | Dark mode visual parity | NOT_STARTED | Token foundation exists, but current-baseline screen-by-screen comparisons are absent |
