...
- アプリ起動直後に「患者 / 家族」を選択する。患者は連携コード入力へ、家族はログインへ遷移する。
...
- 家族モードはメール/パスワードでログインし、患者データを管理する前提（認証基盤の詳細は実装側で扱う）。
- 患者モードは家族が発行する連携コードでリンクし、以後は患者スコープで閲覧できる前提（トークン検証の詳細は後続featureで置換可能）。
